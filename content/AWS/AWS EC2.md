...
- **Why "Instance Limit" is NOT the reason:** If you have reached your service quota (limit) for EC2 instances, AWS will prevent you from even _starting_ the launch process. You will receive an `InstanceLimitExceeded` error immediately in the console or via API, and the instance will never even reach the "Pending" state.
...
- **Zero Inbound Ports:** Unlike all other options, Session Manager requires **no inbound rules** in your Security Groups. You can completely close port 22. The SSM Agent on the instance initiates an _outbound_ connection to the Systems Manager service over HTTPS (port 443).
...
While **Savings Plans** (the newer flexible pricing model) also use 1 or 3-year terms, they allow you to commit to a consistent _amount of spend_ (e.g., $10/hour) rather than specific instance attributes, but the time commitment remains the same.
...
- **Storage Optimized (I/D-series):** These are designed for applications that require high, sequential read/write access to data on local _physical_ storage (like NoSQL databases or data warehousing). They focus on high IOPS, not large RAM capacity.
...
- **Dynamic IP Address:** While a dynamic public IP does expose your instance to the internet, it changes every time the instance stops. The question asks what you should use if _you_ want to choose or maintain a reliable public IP scheme for an instance.
...
**Correct Answer:** Attach one more volume with RAID 1 configuration.

---

### Why this is the correct answer:

When you need maximum data durability and immediate redundancy on host instances beyond what a single EBS volume provides out of the box, software RAID layouts can be configured directly inside the operating system:

- **RAID 1 (Mirroring):** In a **RAID 1** array, data is synchronously mirrored across two or more EBS volumes simultaneously. If one of the underlying volumes experiences an isolated hardware failure or degradation, the instance continues operating seamlessly using the remaining mirrored volume without any data loss or downtime.
- **Fault-Tolerant Focus:** RAID 1 is specifically chosen when your primary architectural objective is **fault tolerance and redundancy**.

---

### RAID Configurations on Amazon EBS:

| RAID Level | Primary Purpose                       | Structural Setup                          | Fault Tolerance Level                                              |
| :--------- | :------------------------------------ | :---------------------------------------- | :----------------------------------------------------------------- |
| **RAID 1** | **Fault Tolerance & Data Redundancy** | Data is mirrored across multiple volumes. | **High:** Can sustain the complete loss of one volume.             |
| **RAID 0** | **Performance & Maximum I/O**         | Data is striped across multiple volumes.  | ❌ **Zero:** Failure of a single volume destroys the entire array. |
